...
import OnePlayerLogic from '@/logics/OnePlayerLogic.js';
...
export function OnePlayerController( state , history) {
    ...
    function play(cell) {
        ...
        //if play win stop the play
        state.gameLogic.checkForWinner();
        if(state.gameLogic.state == Status.FINISHED) return;

        //bot move
        state.gameLogic.playForBot();
        //add move to history
        history.current.push(cell);
